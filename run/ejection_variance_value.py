...
ejection_variance_values = [(0,0,0)]
...
for j in tqdm(range(len(ejection_variance)-1), desc="Processing" , ncols=100, colour='#149414'):
    ...
    aux_ref = vad.mPao_ref

    reached_first_ref_value = 0
    ...
    for i in range(model.n-1):
        ...
        #model.att(i + 1,vad);
        if(round(model.T[i],1)==3.8 and round(ejection_variance[j],1)==0.3):
            print(vad.mPao_v)

        if(np.absolute(vad.mPao_v[i] - aux_ref) <= (aux_ref*0.02) and i!=0):
            if(reached_first_ref_value==0):
                ejection_variance_values.append((round(model.T[i],3),round(ejection_variance[j],2), 90))
                aux_ref = 110
                reached_first_ref_value = 1
            elif(reached_first_ref_value == 1 and vad.mPao_ref==110):
                 ejection_variance_values.append((round(model.T[i],3)-2,round(ejection_variance[j],2), 110))
                 break
            #aux_ref = vad.mPao_ref_values[vad.mPao_ref_index+1]



ejection_variance_values = sorted(ejection_variance_values, key=lambda tupla: tupla[1])
np.savetxt("./testResults/ejection_results_01.csv", ejection_variance_values, delimiter=",", fmt='%.3f')