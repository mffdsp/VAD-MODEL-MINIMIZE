...
class VAD():
  def __init__(self, model, deltaPVAD=0.4):
    ...
    self.Qi = list(np.zeros_like(model.T))
    self.Qo = list(np.zeros_like(model.T))
    self.Qi[0] = 0
    self.Qo[0] = 0
    ...
    self.x = [self.Qi[0], self.Qo[0], self.V_vad]
    ...
    #self.mPao_ref_values = list(np.zeros_like(model.T))
    #self.mPao_ref_values[0] = self.mPao_ref
    #self.mPao_ref_values[1] = 110
    #self.mPao_ref_index = 0
  ...
  def att(self, model, index):
    ...
    self.Qi[index], self.Qo[index], self.Vvad, model.Pao[index], model.Qa[index], model.Vve[index], model.Pas[index], model.Pae[index] = self.x
    self.aux.append(model.Pao[index])

    self.Pp = (self.Vvad - self.Vd_vad)/self.Cp
    self.Px = self.Pp + self.Pex2
    self.Pt = self.Px + self.Rp*(self.Qi[index] - self.Qo[index]) + self.Lp* (self.xdot[0] - self.xdot[1])
  ...
  def valveManager(self, index, model):
    ...
    if self.Di == 0: #IF VALVE IS CLOSE
      ...
      if self.Qi[index] <= 0 and index>self.delay_in+100:
        self.Di = 0
        # self.Do = 1
    ...
    if self.Do == 0:
      ...
      if self.Qo[index] <= 0 and index>self.delay_out+100: 
        self.Do = 0
        # self.Di = 1
